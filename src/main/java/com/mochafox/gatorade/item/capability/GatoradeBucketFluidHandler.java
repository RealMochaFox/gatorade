...
/**
 * Fluid handler implementation for Gatorade Bucket items.
 */
public class GatoradeBucketFluidHandler implements IFluidHandlerItem {
    private static final int CAPACITY = 1000; // mB capacity - standard bucket size

    private ItemStack container;

    public GatoradeBucketFluidHandler(ItemStack container) {
    ...
    /**
     * Gets the fluid currently stored in the bucket
     */
    public FluidStack getFluid() {
        SimpleFluidContent content = container.get(ModDataComponents.GATORADE_BUCKET_FLUID_CONTENT.get());
        if (content == null) {
    ...
    /**
     * Sets the fluid stored in the bucket
     */
    protected void setFluid(FluidStack fluid) {
        if (fluid.isEmpty()) {
            container.remove(ModDataComponents.GATORADE_BUCKET_FLUID_CONTENT.get());
        } else {
            container.set(ModDataComponents.GATORADE_BUCKET_FLUID_CONTENT.get(), SimpleFluidContent.copyOf(fluid));
        }
    }
